@if(count($Hot)>0)
    <section class="cards" id="hot">
        <div class="container">
            <div class="head">
                <h2 class="h2 white">Горячее</h2>
            </div>
            <div class="row" style="justify-content: center;">
                ...
                if($k>=4){$l=3;}
                ?>
                @foreach($Hot as $oneHot)
                        @if(!$oneHot->show_on_site)
                            @continue
                        @endif
                    <div data-id="{{$oneHot->product_id}}" class="col-lg-{{$l}} col-sm-6 productBlock" style="margin-bottom: 60px;">
                        <div class="card">
                            <div class="card__image">
                                <img class="productBlockImage" src="{{$oneHot->image}}" alt="alt">
                                @if(count($oneHot->images))
                                    @foreach($oneHot->images as $img)
                            ...
                            <div class="card__content">
                                <div class="card__content-header">
                                    <div class="card__content-head productBlockTitle">{{$oneHot->title}}</div>
